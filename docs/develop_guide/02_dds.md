...
Q1’s realtime bus is **DDS** (CycloneDDS) for low-latency control and state streaming.

## Why DDS?

- Decoupled publishers/subscribers (no central broker)
- Hard realtime-friendly QoS (best-effort or reliable per topic)
- Education edition: lease `UserCtrl` to publish `rt/lowcmd` and subscribe `rt/lowstate`
...
**ROS tip:** Run ROS 2 on a different DDS domain (or Cyclone config) than the robot bus to avoid discovery storms.
